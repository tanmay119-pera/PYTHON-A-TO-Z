"""
====================================================
 Recursive File/Directory Explorer (Beginner Project)
====================================================
This program uses RECURSION (a function calling itself) to look
inside a folder, and every folder inside that folder, and so on -
then prints everything as a neat indented tree.
...
Module Used: os (built-in Python module, no installation needed)
             It lets us talk to the file system - list folders,
...
def explore_directory(path, depth=0):
    ...
    Parameters:
        ...
        depth (int) -> how many levels deep we currently are.
                        We use this only to add indentation (spaces)
                        so nested items look like a tree.

    Returns:
        (file_count, folder_count) -> total files & folders found
                                       inside this path (including
                                       everything inside sub-folders)
    """
